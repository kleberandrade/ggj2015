Shader "Custom/PlayerTransparent" {
	Properties
{
...
{
    o.Albedo = 1;//fixed3(0.0,0.0,0.0);
    //o.Normal = fixed3(0.0,0.0,1.0);
    float4 Multiply5Bump=_Time * _Speed.xxxx;
    float4 UV_Pan1=float4((IN.uv_BumpMap.xyxy).x,(IN.uv_BumpMap.xyxy).y + Multiply5Bump.x,(IN.uv_BumpMap.xyxy).z,(IN.uv_BumpMap.xyxy).w);
    float4 Multiply1Bump = UV_Pan1 * _Tile.xxxx;
    o.Normal = UnpackNormal(tex2D (_BumpMap, Multiply1Bump.xy));
    o.Emission = 0.0;
    o.Gloss = 0.0;
    o.Specular = 0.0;
    o.Alpha = 1.0;
